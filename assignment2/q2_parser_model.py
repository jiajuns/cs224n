...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_features), name='input_batch')
        self.labels_placeholder = tf.placeholder(tf.float32, (None, self.config.n_classes), name='label')
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embeddings, self.input_placeholder), (-1, self.config.n_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        Hint: Here are the dimensions of the various variables you will need to create
                    ...
                    b2: (n_classes)
        Hint: Note that tf.nn.dropout takess the keep probilitity (not the drop probability) as
            as an argument. The keep probability should be set to self.config.dropout
