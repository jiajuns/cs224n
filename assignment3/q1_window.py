...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_window_features), 'input_batch')
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(tf.nn.embedding_lookup(embeddings, self.input_placeholder), (-1, self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(
                'W', shape=(self.config.n_window_features*self.config.embed_size, self.config.hidden_size),
    ...
    def add_loss_op(self, pred):
        ...
        lable_onehot = tf.one_hot(self.labels_placeholder, self.config.n_classes) # convert labels into one-hot vector
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(pred, lable_onehot), axis=0)
        ### END YOUR CODE
        return loss
