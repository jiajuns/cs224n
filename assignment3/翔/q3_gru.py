...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        rnn = tf.nn.dynamic_rnn(cell,self.inputs_placeholder,dtype = tf.float32)
        preds = tf.nn.sigmoid(rnn[1])
    ...
    def add_training_op(self, loss):
        ...
        gradients = [stuff[0] for stuff in grads_vars]
        origin_norm = tf.global_norm(gradients)

        if self.config.clip_gradients:
            variables = [stuff[1] for stuff in grads_vars]
            gradients, norm = tf.clip_by_global_norm(t_list = gradients, clip_norm = self.config.max_grad_norm, use_norm = origin_norm)
            self.grad_norm = tf.global_norm(gradients)
            #final = [(gradients[i], variables[i]) for i in xrange(len(grads_vars))]
            final = zip(gradients, variables)
        else:
            self.grad_norm = origin_norm
            final = grads_vars
        
        train_op = optimizer.apply_gradients(final)
