...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        padded_sentence = [w for w in sentence]
        padded_labels = [l for l in labels]
        mask = [True for i in xrange(len(sentence))]
        if original_length >= max_length:
            padded_sentence = padded_sentence[:max_length]
            padded_labels = padded_labels[:max_length]
            mask = mask[:max_length]
        else:
            padded_sentence.extend([zero_vector]*(max_length-original_length))
            padded_labels.extend([zero_label]*(max_length-original_length))
            mask.extend([False]*(max_length-original_length))
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder=tf.placeholder(dtype = tf.int32, shape = [None, self.config.max_length,self.config.n_features])
        self.labels_placeholder=tf.placeholder(dtype = tf.int32, shape = [None,self.config.max_length])
        self.mask_placeholder=tf.placeholder(dtype = tf.bool, shape = [None,self.config.max_length])
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        vocabs = tf.Variable(self.pretrained_embeddings)
        words = tf.nn.embedding_lookup(vocabs, self.input_placeholder)
        embeddings = tf.reshape(words, [-1, self.config.max_length, self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o, h = cell(x[:,time_step],h,"RNN")
                # print tf.shape(o), tf.shape(self.dropout_placeholder)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        masked_label = tf.boolean_mask(self.labels_placeholder,self.mask_placeholder)
        masked_preds = tf.boolean_mask(preds,self.mask_placeholder)
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(masked_preds,masked_label)
