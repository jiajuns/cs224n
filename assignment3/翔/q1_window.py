...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i in xrange(len(sentence)):
            wordFeatures = []
            for w in xrange(-window_size, window_size+1):
                if i+w < 0 :
                    wordFeatures.extend(start)
                elif i+w >= len(sentence):
                    wordFeatures.extend(end)
                else:
                    wordFeatures.extend(sentence[i+w])    
            
            windowed_data.append((wordFeatures,labels[i]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(words, [-1, self.config.n_window_features*self.config.embed_size])
